...
int main(int argc, char **argv)
{
    ...
    } while (ch != EOF); // after reading existing file ,serial number will be incremented;
    fclose(ptr);
    while (1)
    {
        char choice;
        ...
        case 'd':
            ...
            {
                fseek(ptr, -4, SEEK_CUR); // very complex bruv you can't understand
                ch = fgetc(ptr);
                while (ch != '\n')
                {

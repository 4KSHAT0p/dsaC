...
int main()
{
  ...
  int a;
  // declaring head
  info *first = malloc(sizeof(info)); // head ko free karunga to first bhi free ho jayega? nahi bruh memory allocate ki hai alag alag to free bhi alag alag
  ...
  while (another == 'Y')
  {
    ...
    scanf("%d", &temp->rn);
    getchar();
    printf("Do you want to enter more ?\n"); // next pehle allocate kar diya ek garbage aayegi agar nahi fill kiya usko isliye agar user haan bole tabhi karo allocate varna null karo agle ko
    another = getchar();
    if (another == 'Y')
    {
      temp->next = malloc(sizeof(info)); // before using the next variable we have to allocate memory for that we can just assign temp->next to temp because temp->next is not allocated
      temp = temp->next;
    }
    else
    {
  ...
  // beware of segmentation faults please do carefully
  ...
  info *freel = first; // freeing memory
  while (freel != NULL)
  {
    free(freel);
    freel = freel->next;
...
void LDELETE(info **x, int z) // head pointer k contents change honge; element insert karu ya delete karu irrespective
{
  info *temp;
  if ((*x)->rn == z)
  {
    temp = *x; // why are we taking backup and freeing memory?
    *x = (*x)->next;
    free(temp);
  }
  else
